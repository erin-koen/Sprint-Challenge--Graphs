...
while len(visited)<len(roomGraph):
    ...
    if room not in visited:
        visited[room] = {direction: '?' for direction in player.currentRoom.getExits()}
        
    # check for unexplored exits
    unex = [direction for direction in visited[room] if visited[room][direction] == '?']
    print("UNEX", unex)
    ...
    if len(unex) > 0:
        ...
        if new_room not in visited:
            visited[new_room] = { direction: '?' for direction in player.currentRoom.getExits()}
...
#         self.rooms[room] = {}
#     def add_connection(self, room, direction, destination):
#         # direction_dict = {direction: destination}
#         # print('direction dictionary: ', direction_dict)
#         self.rooms[room][direction] = destination
        
#     def bft(self, exit, room):
#         q = Queue()
#         q.put([(exit, room)])
...
#             rooms = q.get()
#             print(f'Rooms: {rooms}')
#             # get the second item in the tuple from the last item in the list
            
#             v = rooms[-1][1]
#             prev = None
#             if len(rooms)>1:
#                 prev = rooms[-2][1]
#             print(f'Room we are checking: {v}')

#             # will need to do a little bit of debugging here wrt what's being passed
#             for direction, neighbor in self.rooms[v].items():
#                 if neighbor is not "?" and neighbor is not prev:
#                     print(f'neighbor: {neighbor} \n direction: {direction}')
#                     rooms_copy = list(rooms)
#                     rooms_copy.append((direction, neighbor))
#                     q.put(rooms_copy)
#                 else: 
#                     print(f'ELSE BLOCK \n neighbor: {neighbor} \n direction: {direction}')
#                     rooms_copy = list(rooms)
#                     rooms_copy.append((direction, neighbor))
#                     print(f'LIST OF TUPLES: {rooms_copy}')
#                     return rooms_copy
...
#     print('rooms format: ', tracker.rooms)
#     # add current room to tracker if it doesn't exist (should only really be called on first loop)
#     if player.currentRoom.id not in tracker.rooms:
#         tracker.add_room(player.currentRoom.id)
    
#     # get available exits 
#     exits = player.currentRoom.getExits()
#     # Check rooms in directions of exits
#     # if that room is already in tracker, remove from exits and set it to bft_input, to be called only if len(exits) = 0
#     bft_dir_input = None
#     for e in exits:
#         if player.currentRoom.getRoomInDirection(e).id in tracker.rooms:
#             exits.remove(e)
#             bft_dir_input = e
            
#     # if no available exits after trimming out the above, follow path to get back to an undiscovered
#     if len(exits) == 0 :
#         path_back = tracker.bft(bft_dir_input,player.currentRoom.id)
#         print(f'current room DEAD END:{player.currentRoom.id}')
#         print(f'path_back from DEAD END: {path_back}')
#         for direction in path_back:
#             print(f'current room: {player.currentRoom.id}')
#             print('attempted move: ', direction)
#             traversalPath.append(direction[0])
#             player.travel(direction[0])
    
#     # otherwise
#     else:

#         # add all exits as connections with question marks
#         for e in exits:
#             tracker.add_connection(player.currentRoom.id, e, '?')
...
#         # add new room to tracker.rooms
#         tracker.add_room(player.currentRoom.id)

#         # update connections
#         tracker.rooms[prev_room][direction] = player.currentRoom.id
#         tracker.rooms[player.currentRoom.id][opp_dir] = prev_room
